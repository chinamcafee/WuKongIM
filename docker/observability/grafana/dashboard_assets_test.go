	...
	want := []struct {
		...
		},
		{
			...
			metrics: []string{
				...
				"wukongim_webhook_outbox_live_retry_attempts",
			},
		},
		{
	...
	for _, expected := range want {
		...
		case 28:
			...
			gauges := []struct {
				...
				{metric: "wukongim_webhook_outbox_backlog", expr: `max(wukongim_webhook_outbox_backlog{node_name=~"$node_name"}) by (node_name)`},
				{metric: "wukongim_webhook_outbox_dead_letters", expr: `max(wukongim_webhook_outbox_dead_letters{node_name=~"$node_name"}) by (node_name)`},
				{metric: "wukongim_webhook_outbox_live_retry_attempts", expr: `max(wukongim_webhook_outbox_live_retry_attempts{node_name=~"$node_name"}) by (node_name)`},
			}
			for _, gauge := range gauges {
				target := requireTarget(got, gauge.metric)
				if target.Expr != gauge.expr {
		...
		case 29:
			...
			oldestAge := requireTarget(got, "wukongim_webhook_outbox_oldest_age_seconds")
			if wantExpr := `max(wukongim_webhook_outbox_oldest_age_seconds{node_name=~"$node_name"}) by (node_name)`; oldestAge.Expr != wantExpr {
				t.Errorf("panel %q oldest-age query = %q, want %q", got.Title, oldestAge.Expr, wantExpr)
			}
			logicalBytes := requireTarget(got, "wukongim_webhook_outbox_logical_bytes")
			if wantExpr := `max(wukongim_webhook_outbox_logical_bytes{node_name=~"$node_name"}) by (node_name)`; logicalBytes.Expr != wantExpr {
				t.Errorf("panel %q logical-bytes query = %q, want %q", got.Title, logicalBytes.Expr, wantExpr)
			}
			requireUnitOverride(got, "A", "s")
			requireUnitOverride(got, "B", "bytes")
		}
	}
}
