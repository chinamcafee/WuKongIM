...
- Phase 3 publishes the integration overview, architecture, authentication,
  ...
  product security and reliability boundaries: Gateway token authentication is
  enabled by default and validates routed durable device metadata, product HTTP
  routes still require an external trust boundary, critical message webhooks
  use the bounded durable Outbox while online-status delivery remains
  best-effort, and the webhook sender has no built-in signature header.
