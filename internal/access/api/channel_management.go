...
type channelExpireRequest struct {
	ChannelID   string `json:"channel_id"`
	ChannelType uint8  `json:"channel_type"`
...
func (s *Server) registerChannelRoutes() {
	...
	s.engine.POST("/channel/expire", s.handleChannelExpire)
...
func (s *Server) handleChannelExpire(c *gin.Context) {
	var req channelExpireRequest
	if !bindJSON(c, &req) {
		return
	}
	if strings.TrimSpace(req.ChannelID) == "" || req.ChannelType == 0 {
		writeJSONError(c, "channel_id不能为空且channel_type不能为0！")
		return
	}
	var expireAt int64
	if req.ExpireAt != nil {
	...
	if expireAt < 0 {
		writeJSONError(c, "expire_at不能小于0！")
		return
	}
	...
	if req.ChannelType == frame.ChannelTypePerson {
		...
		if req.ToUID != "" {
			var err error
			channelID, err = runtimechannelid.NormalizePersonChannel(req.ChannelID, req.ToUID)
			if err != nil {
	...
	writeMutationResult(c, s.channels.UpdateExpire(c.Request.Context(), channelusecase.ExpireCommand{
		ChannelKey:          channelusecase.ChannelKey{ChannelID: channelID, ChannelType: req.ChannelType},
		ExpireAtUnixSeconds: expireAt,
