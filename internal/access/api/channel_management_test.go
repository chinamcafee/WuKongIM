...
func TestChannelExpireNormalizesPersonalRelationship(t *testing.T) {
	channels := &recordingChannelUsecase{}
	srv := New(Options{Channels: channels})
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/channel/expire", bytes.NewBufferString(`{"channel_id":"user-b","to_uid":"user-a","channel_type":1,"expire_at":1700000000}`))
	req.Header.Set("Content-Type", "application/json")
	...
	if len(channels.updateExpires) != 1 {
		t.Fatalf("expiry calls = %#v, want one", channels.updateExpires)
	}
	got := channels.updateExpires[0]
	wantID := runtimechannelid.EncodePersonChannel("user-a", "user-b")
	if got.ChannelID != wantID || got.ChannelType != frame.ChannelTypePerson || got.ExpireAtUnixSeconds != 1700000000 || !got.CreateIfMissing {
		t.Fatalf("expiry command = %#v, want normalized personal relationship", got)
	}
}
...
type recordingChannelUsecase struct {
	...
	updateExpires        []channelusecase.ExpireCommand
...
func (r *recordingChannelUsecase) UpdateExpire(_ context.Context, cmd channelusecase.ExpireCommand) error {
	r.updateExpires = append(r.updateExpires, cmd)
	return r.err
}
