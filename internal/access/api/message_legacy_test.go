...
func TestSendMessageMapsSubscribersToRequestScopedCommand(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/message/send", bytes.NewBufferString(`{"from_uid":"system","client_msg_no":"cmd-1","payload":"Y21k","subscribers":["u1","u2"],"sync_once":1}`))
	req.Header.Set("Content-Type", "application/json")
...
func TestSendMessageReturnsCompatibleErrors(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "invalid payload", body: `{"from_uid":"u1","channel_id":"g1","channel_type":2,"client_msg_no":"c1","payload":"?"}`, status: http.StatusBadRequest, want: `{"error":"invalid payload"}`},
		{name: "missing usecase", body: `{"from_uid":"u1","channel_id":"g1","channel_type":2,"client_msg_no":"c1","payload":"aGk="}`, status: http.StatusInternalServerError, want: `{"error":"message usecase not configured"}`},
		{name: "request scoped needs sync once", messages: &recordingMessageUsecase{sendErr: messageusecase.ErrRequestSubscribersRequireSyncOnce}, body: `{"from_uid":"u1","client_msg_no":"c1","payload":"aGk=","subscribers":["u2"]}`, status: http.StatusBadRequest, want: `{"error":"request subscribers require sync_once"}`},
		{name: "canceled", messages: &recordingMessageUsecase{sendErr: context.Canceled}, body: `{"from_uid":"u1","channel_id":"g1","channel_type":2,"client_msg_no":"c1","payload":"aGk="}`, status: http.StatusRequestTimeout, want: `{"error":"request canceled"}`},
	} {
...
func TestSendMessageWaitForPersistReturnsDurableReceipt(t *testing.T) {
	messages := &recordingMessageUsecase{sendResult: messageusecase.SendResult{
		MessageID: 42, MessageSeq: 9, Reason: messageusecase.ReasonSuccess, Deduplicated: true,
	}}
	srv := New(Options{Messages: messages})
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/message/send", bytes.NewBufferString(`{"from_uid":"system","channel_id":"g1","channel_type":2,"client_msg_no":"relationship-1","payload":"aGk=","wait_for_persist":1,"persist_timeout_ms":1000}`))
	req.Header.Set("Content-Type", "application/json")
	...
	if !jsonEqual(rec.Body.String(), `{"status":200,"message_id":"42","message_seq":9,"client_msg_no":"relationship-1","deduplicated":1}`) {
		t.Fatalf("body = %q, want durable receipt", rec.Body.String())
	}
}
...
func TestSendMessageRejectsDurableRequestWithoutClientMsgNo(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/message/send", bytes.NewBufferString(`{"from_uid":"system","channel_id":"g1","channel_type":2,"payload":"aGk="}`))
	req.Header.Set("Content-Type", "application/json")
	...
	if !jsonEqual(rec.Body.String(), `{"status":400,"code":"client_msg_no_required","msg":"持久消息的client_msg_no不能为空","retryable":false}`) {
		t.Fatalf("body = %q, want client_msg_no_required", rec.Body.String())
	}
}

func TestSendMessageReturnsIdempotencyConflict(t *testing.T) {
	srv := New(Options{Messages: &recordingMessageUsecase{sendErr: messageusecase.ErrIdempotencyConflict}})
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/message/send", bytes.NewBufferString(`{"from_uid":"system","channel_id":"g1","channel_type":2,"client_msg_no":"relationship-1","payload":"aGk=","wait_for_persist":1}`))
	req.Header.Set("Content-Type", "application/json")
	...
	if !jsonEqual(rec.Body.String(), `{"status":409,"code":"idempotency_conflict","msg":"client_msg_no已被不同消息使用","retryable":false}`) {
		t.Fatalf("body = %q, want idempotency conflict", rec.Body.String())
	}
}
