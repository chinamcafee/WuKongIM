...
const (
	...
	channelAppendErrCodeIdempotencyConflict     = "idempotency_conflict"
)
...
func appendChannelAppendSendResult(dst []byte, result channelappend.SendResult) []byte {
	...
	return appendChannelAppendBool(dst, result.Deduplicated)
}
...
func readChannelAppendSendResult(body []byte, offset int) (channelappend.SendResult, int, error) {
	...
	if result.Deduplicated, offset, err = readChannelAppendBool(body, offset, "channel append result deduplicated"); err != nil {
		return channelappend.SendResult{}, offset, err
	}
	return result, offset, nil
}
...
func readChannelAppendResultError(body []byte, offset int) (error, int, error) {
	...
	case channelAppendErrCodeIdempotencyConflict:
		return channelappend.ErrIdempotencyConflict, offset, nil
...
func channelAppendErrorCode(err error) string {
	...
	case errors.Is(err, channelappend.ErrIdempotencyConflict):
		return channelAppendErrCodeIdempotencyConflict
