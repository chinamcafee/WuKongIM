...
type SendResult struct {
	...
	Reason Reason
	// Deduplicated reports that a prior durable append satisfied this send.
	Deduplicated bool
}
...
type IdempotencyQuery struct {
	// FromUID is compared with the prior message but is not part of the durable key.
	FromUID string
	...
	PayloadHash uint64
	// Setting is a durable protocol setting compared on idempotency hits.
	Setting uint8
	// Topic is the durable protocol topic compared on idempotency hits.
	Topic string
	// Expire is the durable message expiry compared on idempotency hits.
	Expire uint32
	// SyncOnce is a durable framing flag compared on idempotency hits.
	SyncOnce bool
