...
when the client message number resolves inside the canonical channel and the
stored sender, raw payload hash, setting, topic, expiry, SyncOnce, and RedDot
values all match. Any mismatch returns the typed idempotency-conflict error;
matching hits return the original id/sequence as a deduplicated success.
