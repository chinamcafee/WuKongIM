...
// LookupSend returns a prior successful send only when all durable semantics match.
func (s *ChannelIdempotencyStore) LookupSend(ctx context.Context, query channelappend.IdempotencyQuery) (channelappend.SendResult, bool, error) {
	...
	if (query.PayloadHash != 0 && hit.PayloadHash != query.PayloadHash) ||
		hit.Message.FromUID != query.FromUID ||
		hit.Message.Setting != query.Setting ||
		hit.Message.Topic != query.Topic ||
		hit.Message.Expire != query.Expire ||
		hit.Message.SyncOnce != query.SyncOnce ||
		hit.Message.RedDot != query.RedDot {
		return channelappend.SendResult{}, false, channelappend.ErrIdempotencyConflict
	}
