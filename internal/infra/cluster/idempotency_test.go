...
func TestChannelIdempotencyStoreLookupSendMapsCommittedHit(t *testing.T) {
	node := &recordingIdempotencyNode{
		hit: channelstore.IdempotencyHit{
			Message:     channelruntime.Message{MessageID: 42, MessageSeq: 7, FromUID: "u1"},
	...
	if result.MessageID != 42 || result.MessageSeq != 7 || result.Reason != channelappend.ReasonSuccess || !result.Deduplicated {
...
func TestChannelIdempotencyStoreLookupSendRejectsPayloadHashMismatch(t *testing.T) {
	...
	if !errors.Is(err, channelappend.ErrIdempotencyConflict) {
		t.Fatalf("LookupSend() error = %v, want ErrIdempotencyConflict", err)
	}
	if ok {
