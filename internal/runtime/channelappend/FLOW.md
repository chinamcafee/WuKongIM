...
Durable prepared commands require a non-empty `ClientMsgNo`. Idempotency lookup
uses the canonical channel plus that client number as the key and compares the
stored sender, payload hash, setting, topic, expiry, SyncOnce, and RedDot
semantics before returning the original message id and sequence.
...
The writer admits at most one in-flight item for a given `ClientMsgNo` in that
channel. A duplicate already in the pending queue waits for the first attempt
to finish, then either recovers the committed result or returns a typed content
conflict; it is never coalesced into the same physical append batch.
...
previous durable commit for the same canonical-channel/client key, the writer may
perform one semantic idempotency lookup and complete that item from
...
The append payload carries setting, topic, expiry, RedDot, and the `SyncOnce` command marker through the durable
