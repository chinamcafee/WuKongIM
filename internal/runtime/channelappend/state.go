...
type channelState struct {
	...
	inflightClientMsgNos           map[string]struct{}
...
func (s *channelState) nextAppendBatch() (uint64, []preparedSend, bool) {
	...
	if len(s.inflightClientMsgNos) == 0 {
		hasClientMsgNo := false
		for _, item := range s.pendingItems {
			if item.Command.ClientMsgNo != "" {
				hasClientMsgNo = true
				break
			}
		}
	...
	if len(selectedKeys) > 0 && s.inflightClientMsgNos == nil {
		s.inflightClientMsgNos = make(map[string]struct{}, len(selectedKeys))
	}
	for key := range selectedKeys {
		s.inflightClientMsgNos[key] = struct{}{}
	}
	seq := s.nextAppendSeq
	s.nextAppendSeq++
	s.appendInflight++
...
func (s *channelState) finishAppend(items []preparedSend) {
	...
	for _, item := range items {
		delete(s.inflightClientMsgNos, item.Command.ClientMsgNo)
	}
	if len(s.inflightClientMsgNos) == 0 {
		s.inflightClientMsgNos = nil
	}
}
