...
func TestChannelStateSerializesMatchingClientMessageNumbers(t *testing.T) {
	state := newChannelState(AuthorityTarget{ChannelID: ChannelID{ID: "room", Type: 2}}, channelStateLimits{appendInflightLimit: 2})
	state.enqueuePrepared([]preparedSend{
		{Command: SendCommand{ClientMsgNo: "same"}},
		{Command: SendCommand{ClientMsgNo: "other"}},
		{Command: SendCommand{ClientMsgNo: "same"}},
	})

	_, first, ok := state.nextAppendBatch()
	if !ok || len(first) != 2 || first[0].Command.ClientMsgNo != "same" || first[1].Command.ClientMsgNo != "other" {
		t.Fatalf("first batch = %#v, want one item per idempotency key", first)
	}
	if _, _, ok := state.nextAppendBatch(); ok {
		t.Fatal("second matching key started while the first was in flight")
	}
	state.finishAppend(first)
	_, second, ok := state.nextAppendBatch()
	if !ok || len(second) != 1 || second[0].Command.ClientMsgNo != "same" {
		t.Fatalf("second batch = %#v, want deferred matching key", second)
	}
}
