...
func TestRuntimePlanAdmissionTransfersSharedImmutableStorage(t *testing.T) {
	resolver := &capturingPlanPresenceResolver{
		...
		captured: make(chan []onlinedelivery.RecipientTargetBatch, 1),
	}
	...
	select {
	case captured := <-resolver.captured:
		if &captured[0] != &targets[0] {
			t.Fatal("admission cloned target storage")
		}
		if &captured[0].Recipients[0] != &recipients[0] {
			t.Fatal("admission cloned recipient storage")
		}
	case <-time.After(time.Second):
		t.Fatal("timed out waiting for captured plan targets")
	}
}
...
func TestRuntimeFullQueueWaitsAndHonorsCallerCancellation(t *testing.T) {
	...
	admissionCtx := &selectEntryContext{Context: context.Background(), entered: make(chan struct{})}
	waiting := make(chan error, 1)
	go func() {
		waiting <- runtime.EnqueueRecipientDeliveryPlan(admissionCtx, runtimePlanForTest(30))
	}()
	select {
	case <-admissionCtx.entered:
	case <-time.After(time.Second):
		t.Fatal("waiting admission did not reach the full-queue select")
	}
	close(release)
	select {
	case err := <-waiting:
...
func TestRuntimeStopDrainsAcceptedPlanBeforeCancel(t *testing.T) {
	...
	runtime := NewRuntime(RuntimeOptions{
		...
		SessionWriter: localSessionWriterFunc(func(ctx context.Context, write LocalSessionWrite) SessionWriteResult {
			startedOnce.Do(func() { close(started) })
			select {
			case <-ctx.Done():
				return SessionWriteResult{Disposition: SessionWriteRetryable, Err: ctx.Err()}
			case <-release:
				completed <- write.Event.MessageID
				return SessionWriteResult{Disposition: SessionWriteAccepted}
			}
		}),
	})
	...
	if err := runtime.EnqueueRecipientDeliveryPlan(context.Background(), runtimePlanForTest(28)); err != nil {
		t.Fatalf("second EnqueueRecipientDeliveryPlan() error = %v", err)
	}
	...
	waitForRuntimeState(t, runtime, runtimeClosing)
	close(release)
	got := make([]uint64, 0, 2)
	for len(got) < 2 {
		select {
		case messageID := <-completed:
			got = append(got, messageID)
		case <-time.After(time.Second):
			t.Fatalf("completed plans = %#v, want accepted in-flight and queued plans", got)
		}
	}
	if !reflect.DeepEqual(got, []uint64{27, 28}) {
		t.Fatalf("completed plans = %#v, want accepted in-flight and queued plans", got)
	}
	select {
	case err := <-stopResult:
	...
	case <-time.After(time.Second):
		t.Fatal("Stop() did not return after draining accepted plans")
	}
}
...
func TestRuntimeContinuesLaterOwnerBatchAfterRetryExhaustion(t *testing.T) {
	...
	runtime := NewRuntime(RuntimeOptions{
		...
		RemoteOwnerPusher: remoteOwnerPusherFunc(func(_ context.Context, push onlinedelivery.OwnerPush) (onlinedelivery.OwnerPushResult, error) {
			pushesMu.Lock()
			pushedUIDs = append(pushedUIDs, push.Routes[0].UID)
			call := len(pushedUIDs)
			pushesMu.Unlock()
			if call == 1 {
	...
	if !errors.Is(err, ErrOwnerPushRetryExhausted) {
		t.Fatalf("processPlan() error = %v, want ErrOwnerPushRetryExhausted", err)
	}
	pushesMu.Lock()
	got := append([]string(nil), pushedUIDs...)
	pushesMu.Unlock()
	if !reflect.DeepEqual(got, []string{"u1", "u2"}) {
		t.Fatalf("owner pushes = %#v, want failed batch then later sibling batch", got)
	}
}

func TestRuntimeRejectsMissingPresenceResolver(t *testing.T) {
	runtime := NewRuntime(RuntimeOptions{LocalNodeID: 1})
	if err := runtime.processPlan(context.Background(), runtimePlanForTest(29)); !errors.Is(err, ErrPresenceResolverUnavailable) {
		t.Fatalf("processPlan() error = %v, want ErrPresenceResolverUnavailable", err)
	}
}

func TestRuntimeRejectsPushWhenLocalNodeUnknown(t *testing.T) {
	runtime := NewRuntime(RuntimeOptions{})
	startRuntimeForTest(t, runtime)
	_, err := runtime.PushOwner(context.Background(), onlinedelivery.OwnerPush{
		OwnerNodeID: 1,
	...
	if !errors.Is(err, ErrOwnerPushNotLocal) {
		t.Fatalf("PushOwner() error = %v, want ErrOwnerPushNotLocal", err)
	}
}

func TestRuntimeMissingSessionWriterKeepsRouteRetryable(t *testing.T) {
	runtime := NewRuntime(RuntimeOptions{LocalNodeID: 1})
	startRuntimeForTest(t, runtime)
	result, err := runtime.PushOwner(context.Background(), onlinedelivery.OwnerPush{
		OwnerNodeID: 1,
...
func TestRuntimeRoutesLocalAndRemoteOwnersThroughSameOwnerSeam(t *testing.T) {
...
type capturingPlanPresenceResolver struct {
	...
	called   chan struct{}
	captured chan []onlinedelivery.RecipientTargetBatch
}

func (r *capturingPlanPresenceResolver) EndpointsByTargets(_ context.Context, targets []onlinedelivery.RecipientTargetBatch) []TargetPresenceResult {
	select {
...
func waitForRuntimeState(t *testing.T, runtime *Runtime, want runtimeState) {
	t.Helper()
	deadline := time.Now().Add(time.Second)
	for {
		runtime.mu.Lock()
		...
		if time.Now().After(deadline) {
			t.Fatalf("runtime state = %d, want %d", got, want)
		}
		goruntime.Gosched()
	}
}
