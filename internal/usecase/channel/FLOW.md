...
bounded subscriber page/chunk iteration. It also owns channel expiry updates;
ordinary metadata updates preserve an existing expiry, while an explicit
expiry command may create a missing personal relationship channel.
...
The Slot metadata subscriber store de-duplicates UIDs inside each mutation and
serializes concurrent writes. Repeated allowlist additions therefore preserve
one row and increment the durable subscriber count only for a genuinely new UID.
