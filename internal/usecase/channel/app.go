...
// UpdateExpire updates an existing channel expiry or creates an allowed personal channel.
func (a *App) UpdateExpire(ctx context.Context, cmd ExpireCommand) error {
	if err := a.requireStore(); err != nil {
		return err
	}
	...
	if errors.Is(err, metadb.ErrNotFound) {
		...
		channel = metadb.Channel{ChannelID: cmd.ChannelID, ChannelType: int64(cmd.ChannelType)}
	} else if err != nil {
		return err
	}
	channel.ExpireAtUnixSeconds = cmd.ExpireAtUnixSeconds
	return a.store.UpsertChannel(ctx, channel)
}
