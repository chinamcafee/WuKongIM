	...
	store := &recordingStore{
		channels: map[string]metadb.Channel{
			recordingChannelKey("g1", 2): {
				...
				SubscriberCount:           12,
				ExpireAtUnixSeconds:       1700000000,
			},
		},
	}
	app := New(Options{Store: store})
...
func TestUpdateExpirePreservesExistingChannelMetadata(t *testing.T) {
	existing := metadb.Channel{ChannelID: "u1@u2", ChannelType: 1, Ban: 1, SubscriberCount: 2}
	store := &recordingStore{channels: map[string]metadb.Channel{recordingChannelKey("u1@u2", 1): existing}}
	app := New(Options{Store: store})

	err := app.UpdateExpire(context.Background(), ExpireCommand{
		ChannelKey: ChannelKey{ChannelID: "u1@u2", ChannelType: 1}, ExpireAtUnixSeconds: 1700000000,
	})

	if err != nil {
		t.Fatalf("UpdateExpire() error = %v", err)
	}
	want := existing
	want.ExpireAtUnixSeconds = 1700000000
	if got := store.upsertChannels; len(got) != 1 || got[0] != want {
		t.Fatalf("upserted channels = %#v, want %#v", got, want)
	}
}

func TestUpdateExpireCreatesMissingPersonalChannelWhenAllowed(t *testing.T) {
	store := &recordingStore{getChannelErr: metadb.ErrNotFound}
	app := New(Options{Store: store})

	err := app.UpdateExpire(context.Background(), ExpireCommand{
		ChannelKey: ChannelKey{ChannelID: "u1@u2", ChannelType: 1}, ExpireAtUnixSeconds: 1700000000, CreateIfMissing: true,
	})

	if err != nil {
		t.Fatalf("UpdateExpire() error = %v", err)
	}
	want := metadb.Channel{ChannelID: "u1@u2", ChannelType: 1, ExpireAtUnixSeconds: 1700000000}
	if got := store.upsertChannels; len(got) != 1 || got[0] != want {
		t.Fatalf("upserted channels = %#v, want %#v", got, want)
	}
}
