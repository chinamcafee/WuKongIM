...
// ExpireCommand updates one channel expiry timestamp.
type ExpireCommand struct {
	ChannelKey
	// ExpireAtUnixSeconds is the absolute expiry timestamp; zero disables expiry.
	ExpireAtUnixSeconds int64
	// CreateIfMissing allows a personal channel to be materialized on its first update.
