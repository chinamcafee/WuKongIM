...
func (a *App) checkPersonSendPermission(ctx context.Context, cmd SendCommand) (Reason, error) {
	channel, err := a.permissions.GetChannelForPermission(ctx, cmd.ChannelID, int64(cmd.ChannelType))
	if err != nil && !errors.Is(err, metadb.ErrNotFound) {
		return ReasonSystemError, err
	}
	if err == nil && a.channelExpired(channel) {
		return ReasonSendBan, nil
	}
...
func (a *App) channelExpired(channel metadb.Channel) bool {
	return channel.ExpireAtUnixSeconds > 0 && a.now().Unix() >= channel.ExpireAtUnixSeconds
}
