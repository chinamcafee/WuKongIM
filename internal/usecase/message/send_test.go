...
func TestSendAppliesLegacyPermissionChecksBeforeSubmitter(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cmd:  SendCommand{FromUID: "u1", ChannelID: "g1", ChannelType: channelTypeGroup, Payload: []byte("hi")},
			configure: func(store *fakePermissionStore) {
				store.channels[permissionKey("g1", int64(channelTypeGroup))] = metadb.Channel{ChannelID: "g1", ChannelType: int64(channelTypeGroup), ExpireAtUnixSeconds: 99}
			},
			opts: func(opts *Options) { opts.Now = func() time.Time { return time.Unix(100, 0) } },
			want: ReasonSendBan,
		},
		{
			name: "group denylist",
		...
		{
			name: "expired personal relationship channel",
			cmd:  SendCommand{FromUID: "u1", ChannelID: "u2", ChannelType: channelTypePerson, Payload: []byte("hi"), NormalizePersonChannel: true},
			configure: func(store *fakePermissionStore) {
				channelID := runtimechannelid.EncodePersonChannel("u1", "u2")
				store.channels[permissionKey(channelID, int64(channelTypePerson))] = metadb.Channel{ChannelID: channelID, ChannelType: int64(channelTypePerson), ExpireAtUnixSeconds: 99}
			},
			opts: func(opts *Options) { opts.Now = func() time.Time { return time.Unix(100, 0) } },
			want: ReasonSendBan,
		},
