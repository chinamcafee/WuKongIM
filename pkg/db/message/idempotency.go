...
// LookupIdempotency returns the indexed message selected by a channel/client-message pair.
...
func (l *ChannelLog) lookupIdempotencyByKey(ctx context.Context, key IdempotencyKey, storageKey []byte) (IdempotencyHit, bool, error) {
	...
	if !ok || row.MessageID != hit.MessageID || row.PayloadHash != hit.PayloadHash || row.ClientMsgNo != key.ClientMsgNo {
