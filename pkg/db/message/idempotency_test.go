...
func TestIdempotencyLookup(t *testing.T) {
	...
	hit, ok, err = log.LookupIdempotency(context.Background(), IdempotencyKey{FromUID: "u2", ClientMsgNo: "same"})
	if err != nil || !ok || hit.MessageID != 61 {
		t.Fatalf("LookupIdempotency() with different sender = (%#v, %v, %v), want channel-scoped hit", hit, ok, err)
	}
...
func TestAppendStrictRejectsDuplicateIdempotency(t *testing.T) {
	...
	_, err := log.Append(context.Background(), []Record{{ID: 72, ClientMsgNo: "same", FromUID: "u2", Payload: []byte("two")}}, AppendOptions{})
