...
func TestMessageIndexListsClientMsgNoNewestFirst(t *testing.T) {
	...
	log := testChannelLog(store)
	if _, err := log.Append(context.Background(), []Record{
		{ID: 21, ClientMsgNo: "same", Payload: []byte("one")},
		{ID: 22, ClientMsgNo: "other", Payload: []byte("two")},
		{ID: 23, ClientMsgNo: "same", Payload: []byte("three")},
		{ID: 24, ClientMsgNo: "same", Payload: []byte("four")},
	}, AppendOptions{}); err != nil {
		t.Fatalf("Append(): %v", err)
	}
