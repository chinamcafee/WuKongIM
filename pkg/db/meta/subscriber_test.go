...
func TestConcurrentDuplicateSubscriberAddsAreIdempotent(t *testing.T) {
	store := openTestMetaStore(t)
	defer store.close(t)
	shard := store.db.HashSlot(6)
	ctx := context.Background()
	channel := Channel{ChannelID: "allowlist-race", ChannelType: 1}
	if err := shard.CreateChannel(ctx, channel); err != nil {
		t.Fatalf("CreateChannel(): %v", err)
	}
	...
	for i := 0; i < workers; i++ {
		...
		go func(version uint64) {
			defer wg.Done()
			errs <- shard.AddSubscribers(ctx, channel.ChannelID, channel.ChannelType, []string{"peer", "peer"}, version)
		}(uint64(i + 1))
	}
	wg.Wait()
	close(errs)
	...
	got, ok, err := shard.GetChannel(ctx, channel.ChannelID, channel.ChannelType)
	if err != nil || !ok {
		t.Fatalf("GetChannel() = (%#v, %v, %v)", got, ok, err)
	}
	if got.SubscriberCount != 1 {
		t.Fatalf("SubscriberCount = %d, want 1", got.SubscriberCount)
	}
	members, err := shard.SnapshotSubscribers(ctx, channel.ChannelID, channel.ChannelType)
	if err != nil || len(members) != 1 || members[0] != "peer" {
		t.Fatalf("SnapshotSubscribers() = (%#v, %v), want one peer", members, err)
	}
}
