...
type Channel struct {
	...
	Large int64
	// ExpireAtUnixSeconds blocks ordinary sends at or after this Unix timestamp; zero means no expiry.
	ExpireAtUnixSeconds int64
...
func encodeChannelValue(channel Channel) []byte {
	...
	value = appendValueInt64(value, channel.ExpireAtUnixSeconds)
	return value
}
...
func decodeChannelValue(channelID string, channelType int64, value []byte) (Channel, error) {
	...
	var expireAtUnixSeconds int64
	if len(rest) > 0 {
		expireAtUnixSeconds, rest, err = readValueInt64(rest)
		if err != nil {
			return Channel{}, err
		}
	}
