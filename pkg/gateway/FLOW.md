...
**不负责**:
- 消息发送、RecvAck、Ping、presence activation 等业务路由 → `internal/access/gateway`。
...
| `core/` | `core.NewServer` → `Server` | 网关核心：listener runtime、session state、CONNECT 异步认证流程、decode/dispatch/write loop、idle tracker、workqueue-backed async runtime |
...
入口: `core.Server.handleAuthFrame` → `asyncRuntime.auth` → `authExecutor` / `workqueue.BoundedPool`
...
- 出站 `RecvPacket` 自动加密；入站 `SendPacket` 在 adapter 内按 session crypto 解密并校验 msg key，并 detach payload bytes so async SEND dispatch can safely retain the decoded packet. 业务 handler 拿到明文 payload 后再按业务 reason 写 SendAck。
...
- `pkg/gateway` 可以依赖 Go 标准库、`pkg/protocol/*`、`pkg/observability/sendtrace`、`pkg/workqueue`、`pkg/wklog` 和外部 transport/runtime 依赖。
- `pkg/gateway` 不得依赖 `internal/*`。
- `internal/access/gateway` 可以依赖 `pkg/gateway`，负责把 session lifecycle / `frame.Frame` 转为 message/presence 用例命令。
- Session value key 是 gateway 与 access/gateway 的窄合约；新增或重命名时需要同步 mapper、lifecycle 和测试。
- 新业务帧支持优先加在 access/gateway；只有 wire format 或连接语义变化才修改 `pkg/gateway`。
