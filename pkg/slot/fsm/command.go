...
func encodeChannelCommand(commandType uint8, ch metadb.Channel) []byte {
	...
	// header + 1 string field + 7 int64 fields
	size := headerSize +
		tlvOverhead + idLen +
		tlvOverhead + 8 +
		tlvOverhead + 8 +
	...
	_ = putInt64Field(buf, off, tagChannelExpireAt, ch.ExpireAtUnixSeconds)
...
func decodeChannel(data []byte) (metadb.Channel, error) {
	...
	for off < len(data) {
		...
		case tagChannelExpireAt:
			if len(value) != 8 {
				return metadb.Channel{}, fmt.Errorf("%w: bad ExpireAtUnixSeconds length", metadb.ErrCorruptValue)
			}
			ch.ExpireAtUnixSeconds = int64(binary.BigEndian.Uint64(value))
		default:
			// Unknown tag — skip for forward compatibility.
		}
	}
