...
| `message` | `test/e2e/message/linku_v3_contract` | Lock the Link-U v3 public API, WKProto v6, token-auth, readiness, and durable Webhook Outbox contract on single- and three-node clusters. | `GOWORK=off go test -tags=e2e ./test/e2e/message/linku_v3_contract -count=1 -timeout 4m -p=1` |
...
| `message` | `test/e2e/message/medium_recipient_hotpath` | Opt-in higher-fidelity local Cloud Medium gate using a real three-node process cluster, WKProto sockets, Raft, Pebble, mixed online/offline recipients, SENDACK/RECV latency, and public pressure metrics. | `WK_E2E_MEDIUM_RECIPIENT_HOTPATH=1 WK_E2E_MEDIUM_RECIPIENT_ENFORCE_ACCEPTANCE=1 GOWORK=off go test -tags=e2e ./test/e2e/message/medium_recipient_hotpath -run TestCloudMediumScaledRecipientHotPath -count=1 -timeout 5m -p=1 -v` |
