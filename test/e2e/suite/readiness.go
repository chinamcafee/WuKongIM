...
func WaitTCPReady(ctx context.Context, addr string) error {
	return waitTCPReady(ctx, addr, nil)
}

// WaitTCPReady waits for a TCP listener while also failing when this child exits.
func (p *NodeProcess) WaitTCPReady(ctx context.Context, addr string) error {
	return waitTCPReady(ctx, addr, p)
}

func waitTCPReady(ctx context.Context, addr string, process *NodeProcess) error {
	ticker := time.NewTicker(readyPollInterval)
	defer ticker.Stop()
	processDone := process.Done()
	...
	var lastErr error
	for {
		if err := processReadinessExitError(process, "TCP readiness"); err != nil {
			return err
		}
		dialer := net.Dialer{}
		...
		case <-ctx.Done():
			...
			return ctx.Err()
		case <-processDone:
			return processReadinessExitErrorAfterDone(process, "TCP readiness")
		case <-ticker.C:
		}
	}
}
